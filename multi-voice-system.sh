...
# Step 1: Copy scripts to Docker container
echo "Installing multi-voice launcher in Docker container..."
docker cp custom/multi-voice-launcher.sh coqui-tts:/app/custom/multi_voice_launcher.sh
docker exec coqui-tts chmod +x /app/custom/multi_voice_launcher.sh

echo "Installing voice dispatcher in Docker container..."
docker cp voice-dispatcher.py coqui-tts:/app/custom/voice_dispatcher.py
docker exec coqui-tts chmod +x /app/custom/voice_dispatcher.py

# Step 2: Create logs directory in Docker container
echo "Creating logs directory in Docker container..."
docker exec coqui-tts mkdir -p /app/logs
